...
**Status:** In progress. Slice 1 implements the shared data model, opt-in config, LLM summarization handler, and manual `memory-lane session-end --confirm` command. Slice 2 adds supported Codex automation through `Stop` only when the latest user message explicitly requests a session summary. Slice 3 adds Claude Code automation through Claude's documented `SessionEnd` hook and has been real-world smoked in Claude Code CLI. Slice 4 adds an explicit pi `/memory session-summary` command using pi's documented command, session manager, and UI APIs. A Codex-shaped `session-end` adapter path exists for tests/future compatibility, but current Codex CLI hooks do not expose a supported `SessionEnd` event. A later slice added Claude/Codex `PreCompact` and native pi `session_before_compact` summaries when confirmation is disabled in config. Automatic pi `agent_end` and `session_shutdown` summarization remain out of scope.

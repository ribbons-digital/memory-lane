...
- v0.2.36 was released from `main` at `1623e67` after PR #66 (`fix: suppress broad continuity recall bodies`). Release workflow `28272568916` passed and published 8 assets at https://github.com/ribbons-digital/memory-lane/releases/tag/v0.2.36. Local pre-release verification passed with `pnpm build`, `pnpm test`, and `git diff --check`.

- Installed-artifact dogfood after `memory-lane upgrade --yes` passed: installed Claude/Codex `UserPromptSubmit` for broad `what should we work on next?` emits 636 chars of continuity guidance with no `## Relevant Memory` and no stale Slice 6a body; installed Claude/Codex `hi` returns `{}`; topic lookup still includes bounded relevant memory. Generated Pi bridge fake-host smoke after upgrade still skips `hi`, routes broad next-work prompts to hidden continuity context with `surface: "continuity"`, and preserves technical `pnpm` relevant-memory injection.

- Phase 21 post-v0.2.35 + memory cleanup exit validation is documented at `docs/superpowers/validation/2026-06-26-phase-21-post-v0.2.35-memory-cleanup-exit-validation.md`. Initial verdict was do not exit Phase 21 yet: low-signal `hi` prompts were suppressed, installed/repo-local continuity pointed to `311283e6`, SessionStart was bounded, summary hygiene queues were clean, and project-scoped cross-project dumps stayed hidden, but Claude/Codex broad prior-work/next-action prompts still injected ordinary recall-selected memory bodies, including stale Slice 6a next-slice guidance. Slice 9 fixed that blocker: broad `project-position`/`next-work` prompt families render continuity guidance without `## Relevant Memory` recall bodies, while topic-specific recall, meaningful technical prompts, low-signal suppression, policy-only/off behavior, explicit recall/get, and Pi continuity behavior remain unchanged.

- Post-v0.2.35 memory hygiene dogfood cleaned the active Memory Lane store: oversized/superseded/prompt-dump/global question-fragment memories were deleted or updated, pending memories were triaged to zero, `memory-lane compact` removed tombstones, and `memory-lane reindex` restored approved semantic coverage to 100%. Cross-project prompt-dump candidates from local-video-brain, template-generator, and Sitewright were verified hidden from the Memory Lane project without `--all`.

- `ROADMAP.md` now records a future learning direction: approval/rejection/deletion/rescope/replace/supersede outcomes should become reviewable signals that steer future memory suggesters' scope/category/kind decisions, without silent self-training, automatic durable policy mutation, or auto-approval.

- v0.2.35 was released from `main` at `7db41b0` after PR #65 (`fix: harden prompt context injection`). Release workflow `28237288923` passed, built packages, ran tests, built binaries, smoke-tested the current-platform binary, generated release notes, and published 8 assets at https://github.com/ribbons-digital/memory-lane/releases/tag/v0.2.35.

- Phase 21 Slice 8 context-pollution hardening is complete and released: generated pi native bridge automatic prompt injection now caps raw recall bodies with `contextPolicyPromptMaxChars` plus a 3000-char fallback, shared Claude/Codex/pi lifecycle injection suppresses low-signal greetings like `hi`, meaningful technical recall prompts remain eligible, and explicit `memory_recall`/`memory_get` tools remain full-fidelity. Opus 4.8 reviewed the design and implementation via `claude --model claude-opus-4-8 -p ...`; blockers were folded into the spec before implementation and final implementation review returned APPROVED. Oversized approved memory `4df383a1` was replaced with compact successor `1098781c` (720 chars) using `memory-lane replace --yes`, preserving the durable checkpoint while superseding the 13k-char source.

- v0.2.34 was released from `main` at `f84ee46` after PR #63 (`feat: add phase 21 summary hygiene`). Release workflow `28223214725` passed, built packages, ran tests, built binaries, smoke-tested the current-platform binary, generated release notes, and published 8 assets at https://github.com/ribbons-digital/memory-lane/releases/tag/v0.2.34.

- Phase 21 Slice 7 summary hygiene is complete and released: `handleSessionEnd` suppresses generated session summaries dominated by operational subagent/orchestrator chatter when no durable project outcome is present, and CLI/MCP review surfaces expose read-only `reviewHygiene` hints for existing pending suspect summaries. Review fixes tightened review-status label detection so standalone approval prose does not count as operational chatter and fixed `Procedure:`/`When:`/`Steps:`/`Pitfall:`/`Verify:` durable section-header matching. The slice remains review-first and non-mutating: no auto-reject/delete/supersede, no schema expansion, no retrieval/ranking changes, and no raw transcript indexing.

- v0.2.33 was released from `main` at `5046d8d` after PR #61 (`fix: harden continuity guidance hygiene`) and PR #62 release handoff sync. It completes the minimal continuity hygiene slice after v0.2.32: dump-like skill/tool-doc bodies are excluded from operating guidance, operating-agreement overlap, and workflow-rule global selection; truncated operating guidance is marked; and CLI/Pi/generated bridge renderers tell agents to inspect full guidance with `memory-lane show <id>`.
...
- v0.2.30 was released from `main` at `a8e7167` after PR #54. The release includes PR #53 postmortem learning capture and PR #54 Pi continuity parity. Release workflow `28146021778` passed, published binaries/assets, and local pre-release verification passed with `pnpm build`, `pnpm test`, and `git diff --check`.
...
- Pi prompt-submit crash fix landed via PR #50/#51 and released as `v0.2.28`: after `v0.2.27`, Pi could start but exited on prompt submit when Memory Lane's generated native-binary CLI bridge found relevant memories. Root cause: the generated `before_agent_start` handler returned `{ message: "..." }` as a string, while Pi expects `{ message: { customType, content, display, details? } }`; the repo-local `packages/pi-adapter` already used the correct shape, but the release/native bridge template did not. Local Pi was repaired immediately by editing `~/.pi/agent/extensions/memory-lane/index.ts` to return an object-shaped `memory-lane` custom message with `display: false`. A regression test now smoke-loads the generated CLI bridge and asserts `before_agent_start` returns Pi's custom-message shape. Follow-up workflow hardening: approved memory `24054c28` records the correction/procedure, approved workflow memory `ecb46217` records the five-point generated-adapter guardrail, and README now requires contract tests, lifecycle branch invocation, host API shape assertions, repo-local vs generated behavior comparison, and installed-artifact lifecycle dogfood before harness adapter/template releases.

- Upgrade reapply fix landed via PR #49 and released as `v0.2.27`: after `v0.2.26`, running `memory-lane upgrade --yes` installed the fixed binary but still rewrote the Pi extension with the old binary-import shim. Root cause: self-upgrade replaces the binary on disk, but the already-running old `memory-lane upgrade` process continued in memory and re-applied harness config using stale installer code. The fix adds an internal `upgrade --reapply-install-manifest` mode and makes normal non-Windows upgrade invoke the freshly installed binary for harness reconfiguration. Local Pi was repaired immediately by running the now-fixed installed binary's `memory-lane init --yes --only pi`, producing the CLI bridge extension and smoke-loading it successfully.

- Pi extension startup fix landed via PR #48 and released as `v0.2.26`: release-style `init`/`upgrade` had generated `~/.pi/agent/extensions/memory-lane/index.ts` to dynamically import the installed native binary (`~/.local/bin/memory-lane`) as if it were JS/TS, causing Pi startup to fail with a parser error (`Unexpected character '�'`). The fix makes local checkout CLI dist installs import `packages/pi-adapter/dist/index.js`, while native-binary installs write a self-contained CLI bridge extension instead of importing the binary.
...
Current branch is `main` at `1623e67` (`fix: suppress broad continuity recall bodies (#66)`). Latest released tag is `v0.2.36`. PR #66 and the v0.2.36 release completed Phase 21 Slice 9 broad continuity injection hygiene after the v0.2.35 context-pollution hardening release.

Phase 21 Handoff-Free Sessions has progressed through the Pi continuity routing and hygiene follow-ups: Slice 1 established the handoff-mode contract, Slice 2 added review-mode proposals, Slice 3 added continuity baseline markers, Slice 4 validated review/baseline behavior, Slice 5/5a added automatic SessionStart handoff selection, Slice 5b validated automatic mode, Slice 6/6a added read-only workstream discovery on continuity, Slice 6b-6e hardened desktop/CLI routing and generated skill writes, hygiene UX added exact-id inspection/scope correction, Pi continuity parity shipped through v0.2.30, continuity typing/read-model fields shipped after PR #57, generated Pi operating-guidance cap alignment shipped in v0.2.32, continuity hygiene hardening shipped in v0.2.33, summary hygiene shipped in v0.2.34, context-pollution hardening shipped in v0.2.35, and broad continuity recall-body suppression shipped in v0.2.36.

The current product/design center remains cross-agent continuity without silent autonomy: bounded context, explicit review, text-free diagnostics where possible, harness-neutral learning, and no silent durable rule mutation. The Memory-as-a-Tool paper analysis is parked at `MEMORY_AS_TOOL_REVIEW.md`; it should inform future review-first consolidation proposals, but should not interrupt the release/validation path without evidence.

Current recommended next step: decide whether to declare Phase 21 complete and move to the next approved roadmap track, or first run a small docs-hygiene slice to compact growing `HANDOFF.md`/`ROADMAP.md` context before new feature work. Phase 21 Slice 7/8/9 context-hygiene validation and installed-artifact dogfood are passing; avoid adding retrieval rewrites, auto-consolidation, silent deletion, schema expansion, raw transcript indexing, or workstream IDs without a new approved slice.
...
- pi read-only lifecycle context injection is implemented via `before_agent_start`; broad continuity prompts route through canonical continuity before recall. Pi lifecycle writes are implemented but intentionally low-noise: `input` saves only explicit memory requests, while `turn_end` and `tool_result` capture higher-signal candidates through shared lifecycle handlers. Automatic pi session-shutdown summaries remain out of scope unless a supported-event design is approved.
...
1. Complete the current continuity hygiene slice: keep skill/tool-doc dumps out of operating guidance and operating-agreement overlap hints, and ensure truncated operating guidance tells agents to inspect the full memory by id before applying it.
2. Validate the hygiene slice with focused core tests, full `pnpm test`, `pnpm build`, `git diff --check`, and a Pi-style continuity output check.
3. Before PR, use the global Opus 4.8 review workflow exactly: invoke `claude --model claude-opus-4-8 -p '<review prompt>'` directly, ask for high-effort thinking in the prompt, and have Opus review the implementation/diff until consensus; do not use subagents for Opus review.
4. Keep future learning/consolidation enhancements harness-neutral. Core/lifecycle should own shared continuity semantics, while adapters for pi, Codex, Claude Code, Cursor, Hermes, etc. should only supply bounded lifecycle evidence and render shared outputs.
...
6. For Codex Desktop MCP setup, keep guidance portable: use absolute paths and avoid `~`, but write placeholders such as `<node-binary>`, `<memory-lane-checkout>/packages/mcp-server/dist/index.js`, and `<project-path>` rather than machine-specific paths. Prefer `memory-lane init` for release-style installs; for local-checkout development setup, point readers to `README.md` → “MCP clients: point at the local server” and `examples/harness-integrations/mcp.md`.

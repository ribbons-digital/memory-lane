...
**Status:** Through Slice 9 is complete and released in `v0.2.36` (`1623e67`, PR #66). Slice 8 context-pollution hardening suppresses low-signal greeting prompt-time injection across shared lifecycle adapters, preserves meaningful technical prompts, caps generated pi bridge automatic recall context using prompt char budgets with fallback, leaves explicit recall/get full-fidelity, and replaced oversized approved memory `4df383a1` with compact successor `1098781c`. Post-v0.2.35 memory cleanup and exit validation are documented in `docs/superpowers/validation/2026-06-26-phase-21-post-v0.2.35-memory-cleanup-exit-validation.md`; initial verdict was do not exit Phase 21 because Claude/Codex broad continuity prompts still injected ordinary recall-selected memory bodies. Slice 9 fixed that blocker: broad prior-work/next-action prompt families now render continuity guidance without `## Relevant Memory` recall bodies, while topic-specific recall, meaningful technical prompts, low-signal suppression, and explicit recall/get remain unchanged. Pre-release verification, release workflow `28272568916`, installed upgrade, and installed-artifact dogfood for Claude/Codex/Pi passed. Phase 21 exit criteria are satisfied for the Slice 7/8/9 context-hygiene track; the next decision is whether to declare Phase 21 complete and move to the next approved roadmap track, or first run a small docs-hygiene slice to compact growing handoff/roadmap context. Slice 1 implements the non-breaking `memory.handoffMode` contract and diagnostics. Slice 2 implements read-only review-mode handoff proposals on existing continuity surfaces. Slice 3 implements a per-project advisory continuity baseline marker so existing SessionStart newer-approved notices can work across sessions. Slice 4 validation (`docs/superpowers/validation/2026-06-22-phase-21-slice-4-handoff-continuity-validation.md`) found the review-mode proposal and cross-session baseline behavior bounded, text-free on diagnostics, non-mutating, and stable across handoff modes; verdict: **Proceed to automatic-mode design**. Slice 5 design (`docs/superpowers/specs/2026-06-22-phase-21-automatic-mode-design.md`) and Slice 5a implementation add automatic-mode SessionStart handoff selection. Slice 5b validation (`docs/superpowers/validation/2026-06-22-phase-21-slice-5b-automatic-mode-validation.md`) found automatic mode approved-only, project-scoped, SessionStart-only, budget-neutral, text-free on diagnostics/policy-only surfaces, and non-mutating; verdict: **Proceed to workstream discovery design/spec**. Slice 6 design (`docs/superpowers/specs/2026-06-22-phase-21-workstream-discovery-design.md`) drafted a read-only continuity-query approach. Slice 6a implementation adds optional `workstreamDiscovery` to existing continuity surfaces (`memory-lane continuity --query`, MCP `memory_continuity({ query })`) using approved current-project memories only. Slice 6b validation (`docs/superpowers/validation/2026-06-23-phase-21-slice-6b-desktop-routing-validation.md`) checked desktop/CLI client routing after v0.2.22; evidence shows Claude CLI + Opus 4.8 routes broad continuity prompts to `memory_continuity`, while user-reported Claude Desktop did not, suggesting desktop-specific tool-routing/configuration differences. App-bundled Codex CLI + gpt-5.5 chose Memory Lane-style CLI inspection but not direct MCP calls, and read-only sandboxing exposed the existing home-directory write-test friction on read-only commands. Slice 6c hardens the CLI path by making `status`, `continuity`, and `dashboard` resolve storage without write probes, and corrects bundled skill guidance so broad continuity prompts use `memory_continuity`/`memory-lane continuity --query` before targeted recall. Slice 6d validation (`docs/superpowers/validation/2026-06-23-phase-21-slice-6d-post-6c-dogfood-validation.md`) released `v0.2.23`, confirmed local and installed read-only CLI smokes pass for the three scoped commands, observed Codex `gpt-5.5` reach `memory-lane continuity --query` in one read-only-mode run after upgrade, and observed Claude CLI + Opus 4.8 route a broad continuity prompt to MCP `memory_continuity` in one run. Slice 6e implementation guards the installer/upgrade caveat from Slice 6d: generated Claude/Codex skill writes now resolve symlinked destinations and skip writes that would land inside the Memory Lane source checkout, while preserving hooks/config reapplication and printing visible partial-success warnings. The design spec is `docs/superpowers/specs/2026-06-23-phase-21-slice-6e-installer-skill-destination-hardening-design.md`. `v0.2.24` released that hardening. Follow-up Claude Desktop dogfood then showed broad continuity routing working through the CLI, but exposed hygiene UX gaps around exact-id inspection, confusing `recall --id`, and missing same-id scope correction. The next design spec is `docs/superpowers/specs/2026-06-23-phase-21-memory-hygiene-ux-hardening-design.md`; its implementation adds CLI `show/get`, read-only MCP `memory_get`, clear `recall --id` errors, and CLI same-id `rescope/move` with dry-run/`--yes` gates. Post-release Pi dogfood exposed a release-style installer regression: generated Pi extension shims imported the native `memory-lane` binary as TypeScript. The follow-up fix makes local checkout installs import `packages/pi-adapter/dist/index.js` and native-binary installs write a self-contained CLI bridge extension instead. Follow-up dogfood found self-upgrade still used the old in-memory process to reapply harness configs after installing the new binary; v0.2.27 makes upgrade invoke the freshly installed binary for manifest reapply. A subsequent Pi prompt-submit dogfood found the native-binary CLI bridge returned a string from `before_agent_start` instead of Pi's required custom-message object; `v0.2.28` hardened the generated bridge message shape and documented a generated-adapter release guardrail requiring lifecycle contract tests plus installed-artifact dogfood. Follow-up Pi continuity routing Slice A landed via PR #52 and released as `v0.2.29`: broad Pi prompt-submit questions route through canonical continuity (`memory-lane continuity --query`) before recall in both the generated native bridge and repo-local adapter. Slice B parity is aligned: repo-local adapter and generated bridge both expose save/suggest/recall/get-style explicit memory access, before-agent-start continuity routing, and matching generated-bridge coverage where feasible. Slice C explicit continuity parity shipped in `v0.2.30`: repo-local Pi adds `/memory continuity [query]` and `memory_continuity`, while the generated native bridge adds `memory_continuity` and continues to proxy `/memory continuity ...` through the CLI. Slice D installed-artifact dogfood passed after upgrading to `v0.2.30`: a fresh Pi session used `memory_continuity`, verified git state, and correctly identified Memory Lane continuity / Pi dogfooding at commit `a8e7167` and tag `v0.2.30`. The generated bridge mirrors the shared continuity intent pattern set for release-style installs, including policy-only guidance behavior without memory-body lookup. Post-release installed Pi dogfood succeeded in a fresh Pi session: a broad “what were we last working on?” prompt caused Pi to explicitly inspect Memory Lane continuity and agreements before answering. The postmortem learning follow-up implements the approved design in `docs/superpowers/specs/2026-06-25-postmortem-learning-capture-design.md`: high-confidence debugging postmortems and explicit user challenge/correction turns can queue pending project-scoped `correction`/`procedure` candidates through existing review surfaces, without auto-approval, raw transcript capture, recall-ranking changes, or durable rule mutation. Slice 7 design (`docs/superpowers/specs/2026-06-26-phase-21-slice-7-summary-hygiene-design.md`) and implementation add deterministic generated-summary hygiene: operational-only subagent/orchestrator session summaries are suppressed before writing, while existing pending suspect summaries get read-only `reviewHygiene` hints on CLI/MCP review surfaces. No schema expansion, cleanup mutation, recall ranking, lifecycle injection, or workstream IDs were added. Slice 8 design (`docs/superpowers/specs/2026-06-26-phase-21-slice-8-context-pollution-hardening-design.md`) and implementation harden automatic prompt-time context pollution without changing explicit recall semantics. `manual` is the default inspection-first mode. `review` is behavior-active only for bounded proposals assembled from existing pending continuity candidates; it does not generate new records or inject lifecycle context. `automatic` is behavior-active only for approved, budget-neutral SessionStart handoff selection and remains subordinate to `memory.contextPolicy`. Workstream discovery is read-only and does not add persisted workstream ids, raw transcript indexing, retrieval rewrites, lifecycle injection, or mutation behavior.
...
4. In `automatic` mode, approved session summaries, checkpoint memories, and relevant global preferences are eligible for budgeted injection at the next `SessionStart` alongside baseline memories. **Slice 5a complete: automatic mode prioritizes at most one approved current-project `session_summary` or `project_checkpoint` handoff pointer inside the existing SessionStart budget, emits text-free policy-only guidance, reports text-free diagnostics, and leaves global preferences to existing bounded preference layering. Slice 5b validation complete: automatic mode passed isolated lifecycle/CLI/MCP validation; live desktop-client dogfooding remains a useful future check but does not block workstream discovery design.**
...
6. Add natural-language workstream discovery for queries like “resume building X” or “find where we implemented X,” using approved session summaries, checkpoint memories, provenance, PR/branch references when available, and revision relationships as pointers rather than raw transcript search. **Slice 6a implemented and released in v0.2.22: read-only query-specific `workstreamDiscovery` appears on existing continuity surfaces (`memory-lane continuity --query`, MCP `memory_continuity({ query })`) without new persisted workstream ids, retrieval rewrites, lifecycle injection, or mutation behavior. Slice 6b validation checked whether Claude Desktop, Codex Desktop, Claude CLI, and Codex CLI route natural continuity prompts to those surfaces consistently before any further runtime changes. Slice 6c addresses the evidence-backed follow-up: read-only CLI continuity/status/dashboard commands no longer need home storage writability, and skill guidance now treats continuity—not recall—as canonical for broad handoff prompts. Slice 6d post-release dogfood validated that behavior after `v0.2.23` upgrade in direct installed-binary smoke, plus one Codex read-only exec and one Claude CLI MCP routing run. Slice 6e implements that follow-up guard for generated installed-skill writes so symlinked `~/.agents/skills/memory-lane` or `~/.claude/skills/memory-lane` paths cannot overwrite Memory Lane source skill files; normal end-user installs and non-Memory-Lane dotfiles-style git directories remain allowed. The hygiene UX follow-up implements CLI `show/get`, MCP `memory_get`, clear `recall --id` errors, and CLI same-id `rescope/move` with dry-run/`--yes` gates so agents can inspect and correct memory scope without broad JSON scripting or delete-and-resave workarounds. The Pi extension startup follow-up fixes generated release-style Pi shims so they no longer import native binaries as JS/TS; a follow-up upgrade hardening makes self-upgrade reapply harness config by invoking the freshly installed binary so stale in-memory installer code cannot rewrite old shims after replacement. A further Pi prompt-submit follow-up fixes the generated native CLI bridge's `before_agent_start` return shape so injected Memory Lane context is a Pi custom-message object, not a raw string, and adds a project workflow guardrail for generated harness adapter/template changes. Pi continuity routing Slice A then treats Pi as a first-class continuity harness by routing broad prior-work/next-work prompt-submit questions to canonical continuity before recall; the generated bridge matcher is kept aligned with the shared continuity intent pattern set. Slice A shipped in `v0.2.29` and passed post-release installed Pi dogfood. Slice B adapter/bridge parity is aligned where feasible; Slice C explicit continuity access shipped in `v0.2.30` with repo-local `/memory continuity [query]`, repo-local/generated `memory_continuity`, and generated-bridge registration/execution tests; Slice D installed-artifact dogfood passed after upgrade to `v0.2.30`. Next learning work is specified in `docs/superpowers/specs/2026-06-25-postmortem-learning-capture-design.md`: auto-suggested pending correction/procedure memories from high-confidence debugging postmortems and explicit user challenge/correction turns, without auto-approving or mutating durable rules.**
7. Distinguish orchestrator/session-level summaries from subagent task chatter so parallel-agent workflows produce one durable workstream trail instead of many noisy operational memories. **Slice 7 complete: generated `session_summary` candidates dominated by operational subagent/orchestrator chatter are suppressed before writing when no durable project outcome is present, and existing pending suspect summaries receive read-only CLI/MCP `reviewHygiene` hints. No schema expansion, cleanup mutation, recall ranking, lifecycle injection, or workstream IDs were added.**

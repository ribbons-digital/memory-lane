...
Dogfooding evidence from the v0.2.30 Pi Slice D run adds a concrete continuity-ranking/typing case: the installed Pi bridge correctly routed “What were we last working on?” to `memory_continuity` and recovered by checking git, but the read model's `latestApproved.project` selected a newly approved workflow correction about GitHub PR body formatting (`c78cdc00`) ahead of more useful release/checkpoint memories. The design for the next eval-first slice is `docs/superpowers/specs/2026-06-25-continuity-typing-ranking-eval-design.md`, with implementation plan `docs/superpowers/plans/2026-06-25-continuity-typing-ranking-eval.md`. It distinguishes continuity roles before changing ranking behavior: latest project progress/checkpoint, operating agreement/correction/procedure, global workflow context, and query-specific workstream candidates should be typed and presented separately or scored with explicit, testable rules. The first implementation should be read-only/diagnostic or report-only: add fixtures for broad continuity prompts, assert that release/checkpoint/session-summary records beat ordinary corrections for “last worked on” slots, preserve corrections under operating-agreement/procedure surfaces, and avoid changing recall ranking or injection behavior until the labeled corpus shows clear improvement.

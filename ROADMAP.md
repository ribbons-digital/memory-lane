...
6. Add natural-language workstream discovery for queries like “resume building X” or “find where we implemented X,” using approved session summaries, checkpoint memories, provenance, PR/branch references when available, and revision relationships as pointers rather than raw transcript search. **Slice 6a implemented locally: read-only query-specific `workstreamDiscovery` appears on existing continuity surfaces (`memory-lane continuity --query`, MCP `memory_continuity({ query })`) without new persisted workstream ids, retrieval rewrites, lifecycle injection, or mutation behavior.**

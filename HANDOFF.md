...
- Branch `feature/phase-18-preference-layering` implements Phase 18 Slice 1 global preference layering locally: shared lifecycle context selection now separates current-project preferences/content from bounded global preferences for SessionStart and UserPromptSubmit, `memory.contextPolicy` supports optional `preferenceMaxItems` and `preferenceMaxChars`, and README/CONTEXT/ROADMAP document project-specific narrowing and existing inspection surfaces. Existing Claude Code, Codex, and pi lifecycle-output tests include small grouped-context assertions. No new CLI commands, MCP tools/schema surfaces, automatic approvals, preference learning, rescope/delete/supersede behavior, or rich status/doctor/MCP selected/omitted preference diagnostics were added; those diagnostics remain a deferred Phase 18 follow-up.
...
Current branch for this handoff update is `feature/phase-18-preference-layering`. It is ahead of `main` with Phase 18 Slice 1 global preference layering implementation, tests, and docs; use `git describe --tags --always` for the exact local state.
...
Future roadmap/design center: cross-agent continuity without silent autonomy. The logical implementation order is now documented in `ROADMAP.md`: review/dashboard controls first, then Phase 16 freshness/canonical continuity/memory revision, then Phase 17 review-first progress/checkpoint capture, then Phase 18 global preference layering, then harness-neutral learning, then deeper staleness/consolidation, and finally handoff-free sessions. Phase 17 is complete for the current sequence: checkpoint captures are pending-by-default and deduplicated, and approval remains the durable-continuity boundary. Phase 18 Slice 1 is implemented locally to make global personal preferences consistently available without overpowering project state; richer selected/omitted preference diagnostics for status/doctor/MCP remain deferred. Avoid broad silent background writes until preference and staleness controls are strong enough.

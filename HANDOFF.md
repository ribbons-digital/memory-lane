...
Current branch is `main`, synced with `origin/main`, at `6e3728f` (`fix: route pi continuity prompts to continuity (#52)`). The working tree was clean before this documentation sync began. Latest release tag is `v0.2.29`.

Phase 21 Handoff-Free Sessions has progressed through the Pi continuity routing follow-up: Slice 1 established the handoff-mode contract, Slice 2 added review-mode proposals, Slice 3 added continuity baseline markers, Slice 4 validated review/baseline behavior, Slice 5/5a added automatic SessionStart handoff selection, Slice 5b validated automatic mode, Slice 6/6a added read-only workstream discovery on continuity, Slice 6b-6e hardened desktop/CLI routing and generated skill writes, hygiene UX added exact-id inspection/scope correction, and the Pi follow-up sequence released `v0.2.26` through `v0.2.29` for native-binary Pi startup, upgrade reapply, custom-message shape, and continuity routing.

The current product/design center remains cross-agent continuity without silent autonomy: bounded context, explicit review, text-free diagnostics where possible, harness-neutral learning, and no silent durable rule mutation. The next roadmap slice design spec is drafted at `docs/superpowers/specs/2026-06-25-postmortem-learning-capture-design.md`: learning capture from high-confidence debugging postmortems and explicit user challenge/correction turns, producing pending `correction`/`procedure` candidates for review rather than auto-approving or rewriting workflow rules.
...
1. Review and approve or revise `docs/superpowers/specs/2026-06-25-postmortem-learning-capture-design.md`.
2. After spec approval, write an implementation plan before coding. Keep implementation harness-neutral and review-first: generate pending `correction`/`procedure` candidates, but do not auto-approve, silently mutate existing workflow rules, rewrite recall ranking, or add broad transcript capture.
3. Likely implementation scope: deterministic evidence signals, candidate text conventions, dedup/debounce rules, a shared lifecycle helper, and tests proving normal review/continuity surfaces expose the pending candidates through existing paths.

...
- Phase 21 Slice 6d post-6c dogfood validation is in progress on `docs/phase-21-post-6c-dogfood-validation`: `docs/superpowers/validation/2026-06-23-phase-21-slice-6d-post-6c-dogfood-validation.md` records post-merge/release validation for PR #42. `v0.2.23` was released from `062c8c8`; release workflow `27995237667` succeeded with expected assets. Local-build and installed-binary unwritable-home smokes passed for `memory-lane status --json`, `memory-lane continuity --query "what were we last working on?" --json`, and `memory-lane dashboard --json`. Codex `gpt-5.5` in one read-only-mode run reached `memory-lane continuity --query` successfully after upgrade. Claude CLI + Opus 4.8 in one run routed the broad prompt to MCP `memory_continuity`. One caveat emerged: running `memory-lane upgrade --yes` from the repo overwrote the repository-local `skills/memory-lane/SKILL.md` with a short installer template; this was restored and should be considered for a future installer/skill destination hardening slice.
...
Current branch for this handoff update is `docs/phase-21-post-6c-dogfood-validation` in worktree `~/.config/superpowers/worktrees/memory-lane/phase-21-post-6c-dogfood-validation`. It is based on `origin/main` after PR #42 (`062c8c8`) and contains Phase 21 Slice 6d validation docs plus roadmap/handoff updates. Validation evidence includes `sfw pnpm install`, `pnpm build`, direct local-build read-only smoke, `v0.2.23` release workflow `27995237667`, installed-binary read-only smoke after `memory-lane upgrade --yes`, Codex read-only exec, and Claude CLI MCP routing. Final `git diff --check`, PR, and merge cleanup are still needed.
...
1. Finish Phase 21 Slice 6d docs PR: run `git diff --check`, optionally request review, commit, push, open PR, and wait for user merge.
2. After Slice 6d PR lands, sync main, delete local/remote validation branch and worktree, and save a compact merge checkpoint memory.
3. Recommended next candidate: Phase 21 Slice 6e installer/skill destination hardening design, because `memory-lane upgrade --yes` from the repo overwrote repository-local `skills/memory-lane/SKILL.md` with a short installer skill template during dogfood. Alternative evidence-led candidate: broader read-only CLI taxonomy for commands like `recall`/`list`, but the scoped Slice 6c continuity friction is resolved for `status`/`continuity`/`dashboard`.
4. Keep any Slice 6e bounded: distinguish installed user skill destinations from repository package/source skill files; no new MCP tools, no retrieval rewrite, no lifecycle injection, no raw transcript indexing, no persisted workstream ids/schema, and no auto-approval/mutation behavior.

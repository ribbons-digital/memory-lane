...
- Pi continuity routing Slice A is in progress on `fix/pi-continuity-routing`: broad Pi prompt-submit questions such as “What were we last working on?” now route through canonical continuity before recall. The repo-local pi adapter uses `detectContinuityIntent` plus `engine.continuity({ caller: "lifecycle", query: prompt })` to inject a compact hidden `memory-lane` custom message with `surface: "continuity"`; the generated native CLI bridge uses a deterministic broad-continuity prompt matcher and calls `memory-lane continuity --query <prompt> --json` instead of `recall`. Focused RED/GREEN coverage exercises both the generated bridge and repo-local adapter, asserting broad prompts produce `Memory Lane continuity context` and do not call recall in the generated bridge. Known follow-up debt: the self-contained generated bridge matcher is intentionally smaller than the repo-local adapter's exported `detectContinuityIntent`; future work should align or generate the matcher so local and release-style Pi paths cover the same continuity intent families.
...
Current branch for this handoff update is `fix/upgrade-reexec-reapply` in worktree `~/.config/superpowers/worktrees/memory-lane/upgrade-reexec-fix`. It is based on `origin/main` after PR #48 (`6215e50`) and fixes self-upgrade harness reconfiguration so it runs through the newly installed binary rather than stale in-memory code. Focused RED/GREEN tests cover invoking the freshly installed binary with the internal reapply flag. Full verification, PR, and merge cleanup are still needed.
...
Future roadmap/design center: cross-agent continuity without silent autonomy. The logical implementation order is now documented in `ROADMAP.md`: review/dashboard controls first, then freshness/canonical continuity/revision, review-first progress/checkpoint capture, global preference layering, harness-neutral learning, time-aware memory/freshness advisories, Phase 20.5 validation, and then Phase 21 handoff-free sessions. Phase 20 Slices 1-6 are merged and released through `v0.2.20`; Phase 21 work through Slice 5a is released in `v0.2.21`; Slice 6a is released in `v0.2.22`. Phase 21 Slice 1 established the handoff-mode contract, Slice 2 gives review mode a read-only proposal surface assembled from existing pending continuity candidates, Slice 3 adds the cross-session continuity baseline marker, Slice 4 validates those behaviors, Slice 5 designs and implements automatic SessionStart handoff selection, Slice 5b validates automatic mode, Slice 6 designed read-only workstream discovery, Slice 6a implemented existing continuity query surfaces, Slice 6b validated desktop/CLI client routing, Slice 6c hardened read-only CLI continuity commands plus corrected skill routing guidance, Slice 6d validated that fix after release/dogfood, and Slice 6e is designing installer/skill destination hardening for symlinked generated skill writes.
...
1. Finish Phase 21 Slice 6e design PR: run `git diff --check`, request Opus 4.8 review, address findings, commit, push, open PR, and wait for user merge.
2. After Slice 6e design PR lands, sync main, delete local/remote design branch and worktree, and save a compact merge checkpoint memory.
3. Recommended implementation candidate after design approval: a narrow source-tree realpath guard for generated skill writes in `packages/cli/src/installer/config.ts`, plus visible init/upgrade partial-success warning output when a generated skill write is skipped. Tests should cover symlinked Codex/Claude skill destinations into a fake Memory Lane source checkout, normal end-user writes, dotfiles-style git dirs, and upgrade manifest reapply.
4. Keep implementation bounded: no new MCP tools, no retrieval rewrite, no lifecycle injection, no raw transcript indexing, no persisted workstream ids/schema, no generated-skill marker migration, no manifest format changes, and no auto-approval/mutation behavior.
5. Keep future learning enhancements harness-neutral. Core/lifecycle should own shared continuity semantics, while adapters for pi, Codex, Claude Code, Cursor, Hermes, etc. should only supply bounded lifecycle evidence and render shared outputs.

...
- Phase 21 Slice 8 context-pollution hardening is in progress after v0.2.34 dogfood exposed two issues: generated pi native bridge automatic prompt injection rendered raw recall bodies and bypassed prompt char budgets, while shared Claude/Codex/pi lifecycle injection was bounded but too eager for trivial greetings like `hi`. The implementation suppresses low-signal greeting prompt-time injection across shared lifecycle adapters, preserves meaningful technical recall prompts, caps generated pi bridge automatic recall context using `contextPolicyPromptMaxChars` with a 3000-char fallback, and leaves explicit `memory_recall`/`memory_get` tools full-fidelity. Opus 4.8 reviewed the design via `claude --model claude-opus-4-8 -p ...`; its blockers were folded into the spec before implementation. Oversized approved memory `4df383a1` was replaced with compact successor `1098781c` (720 chars) using `memory-lane replace --yes`, preserving the durable checkpoint while superseding the 13k-char source.
...
Current recommended next step: finish Slice 8 verification/review/release, then resume Phase 21 continuity dogfood / exit validation. Validate installed/current behavior across available harnesses, specifically broad continuity prompts, Slice 7 summary hygiene (`reviewHygiene` hints and operational-only summary suppression), and Slice 8 low-signal prompt suppression/budget caps, then decide whether Phase 21 can exit or needs exactly one evidence-backed follow-up. Keep follow-up work review-first and bounded; avoid retrieval rewrites, auto-consolidation, silent deletion, schema expansion, raw transcript indexing, or workstream IDs.

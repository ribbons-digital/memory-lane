...
- Phase 21 Slice 6c read-only CLI sandbox hardening is in progress on `feature/phase-21-readonly-cli-hardening`: it addresses the Slice 6b Codex finding that read-only CLI commands (`memory-lane status`, `memory-lane continuity --query`, `memory-lane dashboard`) could hit the home-directory `.write-test-*` probe in read-only sandboxes before returning useful continuity state. The CLI now resolves storage without write probes for `status`, `continuity`, and `dashboard`, while mutating commands keep writable-path checks/fallback behavior. The bundled Memory Lane skill guidance now routes broad prompts such as “what were we last working on?” to `memory_continuity`/`memory-lane continuity --query` before targeted recall. No MCP requirement was added for Codex; CLI-based Memory Lane continuity remains acceptable.
...
Current branch for this handoff update is `feature/phase-21-readonly-cli-hardening` in worktree `~/.config/superpowers/worktrees/memory-lane/phase-21-readonly-cli-hardening`. It is based on `origin/main` after PR #41 (`f9c5930`) and contains Phase 21 Slice 6c: read-only storage resolution for CLI `status`/`continuity`/`dashboard`, a regression test for unwritable home storage, skill guidance correction for broad continuity prompts, and roadmap/handoff updates. Verification passed with `pnpm build`, `pnpm test`, and `git diff --check`; Opus 4.8 review found no blockers and the low-risk test-isolation suggestion was applied.
...
1. Finish Phase 21 Slice 6c: run full verification (`pnpm build`, relevant tests, `git diff --check`), request Opus 4.8 high-thinking review, address findings, commit, push, and open a PR.
2. Keep Slice 6c bounded: no new MCP tools, no direct-MCP requirement for Codex, no retrieval rewrite, no lifecycle injection, no raw transcript indexing, no persisted workstream ids/schema, and no auto-approval/mutation behavior.
3. After the Slice 6c PR lands, sync main, clean up the feature branch/worktree, create a compact merge checkpoint memory, and decide the next Phase 21 evidence-led slice.
4. Candidate next slice after 6c: rerun a small Codex/Claude continuity dogfood on the released build to confirm broad prompts can reach Memory Lane continuity without read-only CLI sandbox friction; only then consider additional client-specific setup diagnostics.

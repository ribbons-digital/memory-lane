...
- Branch `feature/phase-20-expiration-metadata` implements Phase 20 Slice 1 locally: Memory Lane records and save/suggest surfaces support optional advisory `freshness` metadata (`expiresAt`, `staleAfterDays`, `capturedAt`) with strict validation, CLI/MCP inputs, and compact human list/review labels. This slice intentionally does not add refresh, consolidation, recall/injection filtering, automatic cleanup, LLM stale classification, or lifecycle auto-population.
...
Current branch for this handoff update is `feature/phase-20-expiration-metadata`. It is ahead of `main` with the Phase 20 Slice 1 expiration/freshness metadata implementation, tests, spec, plan, and docs polish; use `git describe --tags --always` for the exact local state.

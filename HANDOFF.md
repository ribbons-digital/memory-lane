...
Current branch for this handoff update is `feature/phase-17-checkpoint-capture`. It is ahead of `main` with the Phase 17 checkpoint-capture implementation and docs finalization; use `git describe --tags --always` for the exact local state.
...
Future roadmap/design center: cross-agent continuity without silent autonomy. The logical implementation order is now documented in `ROADMAP.md`: review/dashboard controls first, then Phase 16 freshness/canonical continuity/memory revision, then Phase 17 review-first progress/checkpoint capture, then Phase 18 global preference layering, then harness-neutral learning, then deeper staleness/consolidation, and finally handoff-free sessions. Phase 17 is complete for the current sequence: checkpoint captures are pending-by-default and deduplicated, and approval remains the durable-continuity boundary. Next, favor Phase 18 work that makes global personal preferences consistently available without overpowering project state. Avoid broad silent background writes until preference and staleness controls are strong enough.

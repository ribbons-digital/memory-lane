...
Current branch is `docs/v0-2-29-handoff-and-learning-design`, based on `main` at `6e3728f` (`fix: route pi continuity prompts to continuity (#52)`). Latest release tag is `v0.2.29`. The postmortem learning capture implementation is complete on this feature branch and needs final verification/PR review before merge.
...
1. Finish final verification for the postmortem learning capture branch: lifecycle tests/build, full build/test, and `git diff --check`.
2. Open a PR for `docs/v0-2-29-handoff-and-learning-design` and wait for user merge; do not merge directly into `main`.

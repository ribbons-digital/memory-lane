...
- Phase 15 MCP review-filter slice is in progress on branch `phase15-mcp-review-filters`: adds matching `memory_review` filters (`kind`, `source`, `provenance`) so desktop MCP clients can inspect pending session summaries and continuity candidates precisely.
...
- Roadmap extended beyond Phase 13 with a continuity-first order: Phase 14 token-aware context policy, Phase 15 review/dashboard controls, Phase 16 cross-session freshness/status, Phase 17 review-first progress/checkpoint capture, Phase 18 global preference layering, Phase 19 harness-neutral learning enhancements, Phase 20 time-aware memory/consolidation, and Phase 21 handoff-free sessions. New automation remains opt-in/review-first by default.
...
Phase 15 noise-reduction follow-up is complete: suspect-review output is compact/actionable, pi `input` autosave is explicit-memory-request only, historical JSONL rows are more robust, and `doctor` now surfaces skipped-row diagnostics. Phase 15 dashboard slice is merged via PR #2, and CLI review filters/prettier review output merged via PR #5. Active Phase 15 branch: `phase15-mcp-review-filters` for matching MCP review filters.

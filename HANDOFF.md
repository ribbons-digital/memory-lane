...
- Branch `feature/phase-19-workflow-corrections` implements Phase 19 Slice 1 locally: Memory Lane now supports `correction` and `procedure` memory kinds, detects explicit user workflow/process corrections from bounded Stop context, saves compact pending project-scoped `correction` candidates, deduplicates against existing correction/procedure/workflow-rule memories, and surfaces correction candidates through existing review and continuity paths. Approved workflow-like corrections/procedures can participate in operating-agreement discovery while explicit `workflow_rule` memories remain preferred. Focused core, lifecycle, CLI tests and build have passed so far; final full verification/review/PR are pending.
...
Current branch for this handoff update is `feature/phase-19-workflow-corrections`. It is ahead of `main` with the Phase 19 workflow correction capture implementation, tests, spec, plan, and docs polish; use `git describe --tags --always` for the exact local state.

...
- Pi continuity Slice C parity is implemented on `fix/pi-continuity-parity`: repo-local Pi now exposes `/memory continuity [query]` and a `memory_continuity` tool, and the generated native-binary Pi bridge exposes `memory_continuity` while continuing to proxy `/memory continuity ...` through the CLI. Tests cover repo-local registration/command/tool execution and generated bridge registration/execution. This follows the already-released Slice A routing work; Slice D installed-artifact dogfood remains to be rerun after this branch is merged and released.

- Postmortem learning capture was merged via PR #53 into `main` at `d177d34` after final lifecycle/build/test validation: Stop lifecycle detects high-confidence debugging postmortems and explicit user challenge/correction turns using loose natural-language signals plus strict evidence gates, then queues compact pending project-scoped `correction`/`procedure` candidates through existing review paths. The slice adds no LLM classifier, no auto-approval, no raw transcript capture, no new commands/tools, and no recall-ranking changes. No release has been cut yet after PR #53.

- Pi continuity routing Slice A landed via PR #52 and released as `v0.2.29`: broad Pi prompt-submit questions such as “What were we last working on?” now route through canonical continuity before recall. Main is synced past `d177d34` (`Add review-first postmortem learning capture (#53)`); this branch is `fix/pi-continuity-parity`. Post-release installed Pi dogfood succeeded in a fresh Pi session: the prompt “what were we last working on?” caused Pi to explicitly inspect Memory Lane continuity with `memory-lane continuity --query ... --json` and `memory-lane agreements --json` before answering the correct project status. The repo-local pi adapter uses `detectContinuityIntent` plus `engine.continuity({ caller: "lifecycle", query: prompt })` to inject a compact hidden `memory-lane` custom message with `surface: "continuity"`; the generated native CLI bridge mirrors the shared broad-continuity prompt pattern set and calls `memory-lane continuity --query <prompt> --json` instead of `recall`. Focused coverage exercises both generated bridge and repo-local adapter behavior. Review follow-up aligned the generated bridge matcher with the shared `detectContinuityIntent` pattern set, added `where did we leave off?` to the shared detector, and preserved `policy-only` as guidance-only instead of treating it like `off`.
...
Current branch is `fix/pi-continuity-parity`, based on `main` after PR #53 (`d177d34`, `Add review-first postmortem learning capture (#53)`). Latest release tag is `v0.2.29`; no release has been cut for postmortem learning or this Pi parity cleanup yet. This branch aligns the planned Pi continuity follow-up slices by adding explicit repo-local/generated `memory_continuity` parity and updating docs/tests before PR review.

Phase 21 Handoff-Free Sessions has progressed through the Pi continuity routing follow-up: Slice 1 established the handoff-mode contract, Slice 2 added review-mode proposals, Slice 3 added continuity baseline markers, Slice 4 validated review/baseline behavior, Slice 5/5a added automatic SessionStart handoff selection, Slice 5b validated automatic mode, Slice 6/6a added read-only workstream discovery on continuity, Slice 6b-6e hardened desktop/CLI routing and generated skill writes, hygiene UX added exact-id inspection/scope correction, and the Pi follow-up sequence released `v0.2.26` through `v0.2.29` for native-binary Pi startup, upgrade reapply, custom-message shape, and Slice A continuity routing. Slice B repo-local/generated parity is aligned where feasible; Slice C explicit `memory_continuity` parity is implemented on this branch; Slice D installed-artifact dogfood should be rerun after release.
...
1. Open a PR for `fix/pi-continuity-parity` and wait for user merge; do not merge directly into `main`.
2. After merge/release, rerun installed-artifact Pi dogfood for Slice C: verify generated native bridge registration includes `memory_continuity`, `/memory continuity ...` proxies to the CLI, and broad `before_agent_start` continuity prompts still route to `memory-lane continuity --query` before recall.
3. After merge/release, dogfood a high-confidence debugging postmortem or explicit challenge turn through a real Stop lifecycle and review the pending `correction`/`procedure` candidate before approval.
4. Keep future learning enhancements harness-neutral. Core/lifecycle should own shared continuity semantics, while adapters for pi, Codex, Claude Code, Cursor, Hermes, etc. should only supply bounded lifecycle evidence and render shared outputs.

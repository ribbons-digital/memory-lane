...
- Branch `feature/phase-18-preference-diagnostics` implements the Phase 18 preference diagnostics follow-up locally: core `preferenceDiagnostics` metadata now reports visible/current-project/global preference counts and baseline SessionStart selected/omitted preference-cap counts; `MemoryEngine.doctor()`, CLI `status`/`doctor`, and MCP `memory_status` expose the diagnostics without preference bodies, ids, previews, new CLI commands, new MCP tools, or lifecycle behavior changes. Focused core, CLI, MCP, lifecycle tests and build have passed so far; final full verification/review/PR are pending.
...
Current branch for this handoff update is `feature/phase-18-preference-diagnostics`. It is ahead of `main` with the Phase 18 preference diagnostics follow-up implementation, tests, spec, plan, and docs polish; use `git describe --tags --always` for the exact local state.
...
Future roadmap/design center: cross-agent continuity without silent autonomy. The logical implementation order is now documented in `ROADMAP.md`: review/dashboard controls first, then Phase 16 freshness/canonical continuity/memory revision, then Phase 17 review-first progress/checkpoint capture, then Phase 18 global preference layering, then harness-neutral learning, then deeper staleness/consolidation, and finally handoff-free sessions. Phase 17 is complete for the current sequence: checkpoint captures are pending-by-default and deduplicated, and approval remains the durable-continuity boundary. Phase 18 Slice 1 is merged; the current diagnostics follow-up makes preference influence inspectable through existing status/doctor/MCP metadata without exposing preference text. Avoid broad silent background writes until preference and staleness controls are strong enough.

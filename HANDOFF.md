...
Future roadmap/design center: cross-agent continuity without silent autonomy. The logical implementation order is now documented in `ROADMAP.md`: review/dashboard controls first, then Phase 16 freshness/canonical continuity/memory revision, then review-first progress/checkpoint capture, then global preference layering, then harness-neutral learning, then deeper staleness/consolidation, and finally handoff-free sessions. Favor features that let a session discover newer approved project progress from other sessions/harnesses, consistently apply global personal preferences, identify the current canonical workflow/operating-agreement memory, and prompt/suggest reviewable memory updates when important continuity events happen. Avoid broad silent background writes until review/dashboard, token policy, freshness, canonical/revision, checkpoint, preference, and staleness controls are strong enough.

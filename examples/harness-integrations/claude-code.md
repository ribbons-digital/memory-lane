...
`PreCompact` runs immediately before Claude Code compacts context. When `memory.sessionEndSummary.enabled` is configured, `memory.sessionEndSummary.requireConfirmation` is `false`, and `memory.preCompactSummary.enabled` is omitted or not `false`, it can save a pending pre-compact `session_summary` for review without blocking compaction. Set `memory.preCompactSummary.enabled` to `false` to opt out.

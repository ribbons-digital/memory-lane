...
2. Configure `PreCompact` with `memory.sessionEndSummary.requireConfirmation` set to `false` and keep `memory.preCompactSummary.enabled` omitted or not `false` so Codex can queue a pending pre-compact summary before compaction. Set `memory.preCompactSummary.enabled` to `false` to opt out without disabling manual or explicit-intent summaries.

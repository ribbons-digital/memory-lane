...
describe("uninstall", () => {
  ...
  it("rejects malformed selective uninstall flags before removing anything", () => {
    ...
    for (const args of [
      ...
      assert.equal(result.stdout, "")
      assert.match(result.stderr, /Usage: memory-lane uninstall --only omp|selective uninstall supports only/u)
  ...
  it("refuses malformed manifests and tampered OMP paths without deleting", () => {
    ...
    assert.match(malformed.stderr, /Invalid JSON in install manifest/u)
    assert.equal(fs.existsSync(binaryPath), true)
    ...
    assert.match(unsafe.stderr, /Refusing to manage an unexpected OMP extension path/u)

...
function sessionMessagesFromPiCompactionEvent(event: any, ctx: any): any[] {`,
  ...
  const queueLifecycleNotice = (stdout: string, ctx: any, turnId?: string) => {
    ...
    const advisory = typeof data?.advisory?.message === "string" ? data.advisory.message : undefined
  ...
  pi.on("session_compact", async (event: any, ctx: any) => {
    ...
    void runMemoryLaneWithInput(["pi", "pre-compact", "--json"], JSON.stringify(payload), ctx)
      .then((stdout) => {
        queueLifecycleNotice(stdout, ctx, deferred.turnId)
      })
      .catch((error) => {
  ...
  pi.on("turn_end", async (event: unknown, ctx: unknown) => {
    ...
    try {
      const stdout = await runMemoryLaneWithInput(["pi", "turn-end", "--json"], JSON.stringify({
        event: "turn-end",
        cwd: bridgeCwd(ctx),
        session_id: piSessionId(ctx),
      ...
      queueLifecycleNotice(stdout, ctx, normalized.turnId)
    } catch (error) {
      if (piHookDebugEnabled()) notify(ctx, error instanceof Error ? error.message : String(error), "warning")
    }
    return undefined
  ...
  pi.on("tool_result", async (event: unknown, ctx: unknown) => {
    ...
    try {
      const turnId = bridgeTurnId(record)
      const stdout = await runMemoryLaneWithInput(["pi", "post-tool-use", "--json"], JSON.stringify({
        event: "post-tool-use",
        cwd: bridgeCwd(ctx),
        session_id: piSessionId(ctx),
        turn_id: turnId,
      ...
      queueLifecycleNotice(stdout, ctx, turnId)
    } catch (error) {
      if (piHookDebugEnabled()) notify(ctx, error instanceof Error ? error.message : String(error), "warning")
    }
    return undefined
...
`
  return `${base.slice(0, closing)}${lifecycleHandlers}${base.slice(closing).replace("\n", `\n${helpers}`)}`
}
...
export function ompExtensionSource(binaryPath: string): string {
  return piCliBridgeSource(binaryPath)
}
...
export function installOmp(options: InitOptions, recordedConfigPath?: string): IntegrationResult {
  ...
  fs.writeFileSync(validated.value, ompExtensionSource(options.binaryPath), "utf8")

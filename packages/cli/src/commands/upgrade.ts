...
function installedBinaryPath(homeDir: string, isWindows: boolean): string {
  return isWindows ? path.join(homeDir, "bin", "memory-lane.exe") : path.join(homeDir, ".local", "bin", "memory-lane")
}
...
export async function handleUpgrade(argv: string[]): Promise<void> {
 ...
 "memory-lane-upgrade-"))
  ...
  const binaryPath = installedBinaryPath(homeDir, isWindows)
  ...
  try {
    if (reapplyOnly) {
      ...
      if (configuredCount === 0) {
        console.log("No previous harness configs were reapplied. Run `memory-lane init` to set up integrations.")
      } else {

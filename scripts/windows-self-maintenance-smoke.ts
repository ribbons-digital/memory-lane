...
async function main(): Promise<void> {
  ...
  const identityCheck = spawnSync("powershell.exe", [
    ...
    [
      ...
      "$script:ownerWriteAttempts = 0",
      "function Write-Upgrade-Lock-Owner { param($temporaryOwnerPath, $ownerPath, $json); $script:ownerWriteAttempts++; if ($script:ownerWriteAttempts -lt 6) { throw 'transient owner write failure' }; [IO.File]::WriteAllText($temporaryOwnerPath, $json, [Text.UTF8Encoding]::new($false)); [IO.File]::Replace($temporaryOwnerPath, $ownerPath, $null) }",
      "try { if (-not (Update-Upgrade-Lock-Lease)) { throw 'transient lease refresh was treated as lease loss' }; if ($script:ownerReadAttempts -lt 8) { throw 'lease refresh did not retry transient reads and writes' }; if ($script:ownerWriteAttempts -ne 6) { throw 'lease refresh reported success before owner write succeeded' }; $updatedOwner = [IO.File]::ReadAllText((Join-Path $leaseRoot 'owner')) | ConvertFrom-Json; if ($updatedOwner.processStartedAt -ne $startedAt) { throw 'recovery owner omitted stable process identity' } } finally { Remove-Item -LiteralPath $leaseRoot -Recurse -Force }",

...
Codex `PreCompact` can save a pending session summary before context compaction when `memory.sessionEndSummary.enabled` is configured; set `memory.preCompactSummary.enabled` to `false` to opt out. Codex `Stop` can produce a session summary only when the latest user message explicitly asks for it, such as "remember this session" or "summarize this session to memory".
